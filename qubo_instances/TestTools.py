...
def exhaustiveSearch(numSpins, objFun,
                     stopAtFeasible=False, verbose=True, binary=False):
    """
    Go thru all possible spins to find min
    Use binary = True for QUBO and False for Ising
    """
    ...
    for v in range(2 ** numSpins):
        ...
        if (binary):
            spins = [int(s) for s in format(v, '0{}b'.format(numSpins))]

        if v > progCount * progSpacing:
            progCount = int(np.floor(v / progSpacing) + 1)
            if verbose:
                print('\rConfigurations evaluated: {0:6.2f}%'.format(progCount / 100.0), end='')
    ...
    if verbose:
        print('\rConfigurations evaluated: {0:6.2f}%'.format(100))
